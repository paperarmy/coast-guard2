...
| F-03 | 실시간 환경 패널 (기상·조위·시간대) | High | ✅ 완료 (기상청 ASOS 실측) |
...
| F-07 | STL 시계열 이상탐지 그래프 | Medium | ✅ 완료 (STL 분해 918일 실측) |

### 3.3 감시 자산 (F-08 ~ F-09)

| ID | 기능 | 우선순위 | 상태 |
|----|------|---------|------|
| F-08 | 자산 배치 지도 (드론·TOD·CCTV·해경) | Medium | ✅ 완료 |
...
| F-19 | 실 CVI 재산출 (SHAP 가중치 적용) | Critical | ✅ 완료 (5개 인자 전부 실 데이터) |
...
| ID | 기능 | 우선순위 | 상태 |
|----|------|---------|------|
| F-25 | 1시간 주기 자동 데이터 갱신 (Vercel Cron Jobs) | High | ✅ 완료 (/api/cron/refresh, 매시 정각) |
...
### Phase 1 — Vercel 배포 + 실 데이터 연동 완성 ✅ 완료 (2026-07-22)
